...
| Metric | Value |
|--------|-------|
| Frontend Tests | 294+ tests (29 files) ✅ |
| Backend Tests | 63 tests (90% coverage) ✅ |
| TypeScript Errors | 0 ✅ |
| Build Time | ~1.2s ✅ |
...
- `LessonShell` — 4-column grid layout (scoreCard | pullsCard | rewardCard | regretCard) + topSection + traceSection + vizSection
- `TheoryCard` — Collapsible algorithm explanation
- `TracePanel` — Detailed trace inspection (compact view)
- `LessonControls` — Play/pause, speed buttons
- `Chart*` — Live visualization (LineChart, ArmBar, PullHistogram, BetaDistribution, TreeDiagram, DriftTimeline, ConfidenceEllipse)
...
**Rendering:**
- Custom SVG charts with canvas-based scaling and rendering
...
**State Separation:**
- **Backend owns:** Bandit logic, arm stats, trace history, decision logic (via `decide()`, `update()`)
- **Frontend owns:** UI state (paused, speed, theme), progress tracking, **reward computation** (via `rewardFn`)
- Rewards are computed **client-side** by each lesson's `rewardFn`, not generated by the backend
...
| Method | Path | Purpose |
|--------|------|---------||
| POST | `/api/sessions` | Create session (init bandit) |
| GET | `/api/sessions/{id}` | Get stats & step count |
| POST | `/api/sessions/{id}/step` | Decide on context (returns trace, arm scores) |
| POST | `/api/sessions/{id}/update` | Record reward & learn |
| DELETE | `/api/sessions/{id}` | Clean up session |

**Lesson Extras (6 endpoints):**
| Method | Path | Purpose |
|--------|------|---------||
| POST | `/api/sessions/{id}/arm` | Add arm |
| DELETE | `/api/sessions/{id}/arm/{arm}` | Remove arm |
| POST | `/api/sessions/{id}/drift` | Inject reward drift |
| POST | `/api/sessions/{id}/offline-eval` | IPS/DR/NCIS evaluation |
| GET | `/api/sessions/{id}/cluster-map` | PCA cluster visualization |
| GET | `/api/sessions/{id}/leaf-scores` | CATS tree leaf scores |
...
app/
  ...
  models/
    ...
    trace.py           # TraceEntry, ArmScore models
    eval.py            # Offline evaluation models
  routers/
    session.py         # Core CRUD endpoints (5)
    lesson_extras.py   # Lesson-specific endpoints (6)
  services/
    session_service.py # BanditSessionService, session lifecycle
    trace_builder.py   # TraceBuilder, convert coba output to TraceEntry
  simulators/
    base.py            # RewardFn interface
    context_free.py    # Context-free reward functions
    contextual_linear.py # Linear contextual rewards
    advanced.py        # Advanced rewards (Neural, Forest, GP, etc.)
```
...
- Error responses: 400 (invalid), 404 (not found), 422 (validation), 500 (server)
