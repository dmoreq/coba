...
**Toán học:**
$$ Score_i = \hat{\mu}_i + \alpha \sqrt{\frac{2\ln(N)}{n_i}} $$
Trong đó $N$ là tổng số lượt kéo toàn hệ thống, $n_i$ là số lượt kéo của arm $i$.
...
# Lấy điểm (Không cần truyền context vector)
# Hệ thống sẽ lấy mẫu từ phân phối Beta → giá trị trong [0, 1]
score = model.score(context=None)
print(f"Điểm lấy mẫu: {score * 100:.2f}%")

# Quan sát thành công (reward = 1.0)
# alpha tăng lên → phân phối Beta dịch chuyển sang phải (điểm cao hơn)
model.update(context=None, reward=1.0)

new_score = model.score(context=None)
print(f"Điểm sau khi cập nhật: {new_score * 100:.2f}%")
```
